...
module SparseVector =
    ...
    let preparePositions<'a, 'b, 'c> (clContext: ClContext) workGroupSize opAdd =
        ...
        let preparePositions (op: Expr<'a option -> 'b option -> 'c option>) =
            <@ fun (ndRange: Range1D) vectorLength valuesLength (leftValues: ClArray<'a>) (leftIndices: ClArray<int>) (rightValues: ClArray<'b>) (rightIndices: ClArray<int>) (resultBitmap: ClArray<int>) (resultValues: ClArray<'c>) (resultIndices: ClArray<int>) ->

                    let gid = ndRange.GlobalID0

                    if gid < vectorLength then

                        let (leftValue: 'a option) = //None
                            (%binSearch) valuesLength gid leftIndices leftValues

                        let (rightValue: 'b option) = // None
                            (%binSearch) valuesLength gid rightIndices rightValues
        ...
        fun (processor: MailboxProcessor<_>) (vectorLenght: int) (leftValues: ClArray<'a>) (leftIndices: ClArray<int>) (rightValues: ClArray<'b>) (rightIndices: ClArray<int>) ->
            ...
            let ndRange = Range1D.CreateValid(vectorLenght, workGroupSize)
    ...
    let elementWise<'a, 'b, 'c when 'a: struct and 'b: struct and 'c: struct>
        ...
        fun (processor: MailboxProcessor<_>) (leftVector: ClSparseVector<'a>) (rightVector: ClSparseVector<'b>) ->
            ...
            let bitmap, allValues, allIndices =
                prepare processor leftVector.Size leftVector.Values leftVector.Indices rightVector.Values rightVector.Indices
